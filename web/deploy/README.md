...
| `DATA_BACKEND`            | 読み取り元。既定 `notion`／`sqlite` で SQLite へ切替（`db:import` 後）    |
...
## 3b. 読み取りバックエンド切替（脱 Notion）

- 既定は `DATA_BACKEND=notion`（Notion から読む）。`db:import` で SQLite に取り込んだ後、
  `DATA_BACKEND=sqlite` にして再起動すると SQLite から読む（Notion 非依存）。
- 問題があれば `DATA_BACKEND=notion`（または未設定）に戻して再起動すれば元に戻る。
